...
if (isset($_GET['mail'])){
	...
	include_once('../../db.php');
	if ((filter_var($_GET['mail'], FILTER_VALIDATE_EMAIL))&&($res=mysqli_query('SELECT uid, realname, name FROM users, end_users WHERE email = "'.mysqli_real_escape_string($_GET['mail']).'" AND org = id LIMIT 0,1'))){
		$accts=array();
		...
		if (!empty($accts)){
			...
			mysqli_query ("INSERT INTO lost_pass (accounts,code) VALUES ('".json_encode($accts)."',$code)");
			$id=mysqli_insert_id();
			$msg=
"<html>Greetings!
