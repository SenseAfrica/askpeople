...
if(isset($_POST['HWINPUTS-submit'])){
	...
	if(!isset($_SESSION['submitter_id'])){
		...
		if ($result == 'TRUE') {
			mysql_query('INSERT INTO visitors (step,birth,gender,ed,job,kids) VALUES (5,'.(int)$_POST['HWINPUTS-birth'].','.(int)$_POST['HWINPUTS-gender'].','.(int)$_POST['HWINPUTS-ed'].','.(int)$_POST['HWINPUTS-job'].','.(int)$_POST['HWINPUTS-kids'].')');
			if ($id=mysql_insert_id()) $_SESSION['submitter_id']=(int)$id;
...
if ((!isset($_GET['go'])) || (count($tmp=explode('.',$_GET['go']))<2) || (!is_numeric($tmp[0]))|| (!is_numeric($tmp[1])) || (!($res=mysql_query("SELECT fieldset,name,mail FROM forms_{$tmp[0]} WHERE id = {$tmp[1]} AND public =1 AND active = 1"))) || (!($form=mysql_fetch_assoc($res)))){
	include ("head.php");
