...
sub = function (frm){
	if (typeof(frm.formname)!='undefined') {
		$("#tempform")[0].innerHTML="<legend>"+frm.formname.value.split('<').join('&lt;')+"</legend><fieldset><ol>"+((frm.formemail.value!='<?php echo $_SESSION['user']['email'];?>')?("<i id='HW_Mail'style='display:none'>"+frm.formemail.value+"</i>"):'')+"</ol></fieldset>";
...
sub_new = function (pos){
	...
	*/
	...
	$("#tempform")[0].children[1].children[0].appendChild(li);
...
if ($("#embed")[0].style.display!='block'){
		if ($("#tempform")[0].children[1].children[0].children.length==0) $.Notify({caption: 'Alert!',content: 'Your form is still empty!',timeout: 5000,style: {background: 'red', color: 'white'}});
		else {
			...
			$.post("temp_form.php", {
				...
				content:$("#tempform")[0].children[1].children[0].innerHTML,
		...
		<div class="grid">
			<div class="row">
				...
				<h1>New Survey</h1>
					...
					<span id="tempform"><?php
						if (isset($_SESSION['stored_form'])) echo "<legend>{$_SESSION['stored_form']['name']}</legend><fieldset><ol>{$_SESSION['stored_form']['content']}</ol></fieldset>";
					?></span>
